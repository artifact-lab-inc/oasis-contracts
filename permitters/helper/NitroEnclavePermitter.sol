...
abstract contract BaseNitroEnclavePermitter is Permitter {
    ...
    function _acquireIdentity(
        ...
        uint256 maxDuration = _getPermitMaxDuration(
            identity,
            requester,
            context
        );
    ...
    function _processAttestation(
        ...
        uint256 nbf = block.timestamp -
            _getPermitMaxDuration(identity, requester, context);
        NE.UserData memory userdata = NE.verifyAttestationDocument(
            doc,
            _getPCR(),
            nbf
        );
        if (IdentityId.unwrap(burnt[userdata.nonce]) != 0)
            revert DocAlreadyUsed();
        bytes32 expectedBinding = keccak256(
            abi.encode(
                block.chainid,
                _getIdentityRegistry(),
                identity,
                requester,
                !release
            )
    ...
    function _getPCR() internal view virtual returns (NE.PcrSelector memory);

    function _getPermitMaxDuration(
        IdentityId identity,
        address requester,
        bytes calldata context
...
library NE {
    ...
    function verifyAttestationDocument(
        bytes calldata doc,
        PcrSelector memory pcrs,
        uint256 nbf
        ...
        if (STRICT) {
            ...
            require(
                bytes8(doc[0:8]) == bytes8(0x84_44_a1_01_3822_a0_59),
                "invalid doc"
            );
        }
    ...
    function verifyPayload(
        ...
        PcrSelector memory pcrs,
        uint256 nbf
    ) internal view returns (bytes calldata pk, UserData memory userdata) {
        ...
        if (STRICT) {
            ...
            // Text(var) - 78
            require(
                bytes12(payload[cursor:cursor + 12]) ==
                    bytes12(0xa9_69_6d6f64756c655f6964_78),
                "expected module_id"
            );
        }
        cursor += 12;
        ...
        if (STRICT) {
            ...
            require(
                bytes25(payload[cursor:cursor + 25]) ==
                    bytes25(
                        0x66_646967657374_66_534841333834_69_74696d657374616d70_1b
                    ),
        ...
        uint256 timestamp = uint256(
            uint64(bytes8(payload[cursor:cursor += 8]))
        );
    ...
    function _verifyCerts(
    ...
    ) internal view returns (bytes calldata publicKey, uint256 adv) {
        uint256 cursor;
        ...
        if (STRICT) {
            ...
            // Value: Bytes(long) - 59
            require(
                bytes13(input[cursor:cursor + 13]) ==
                    bytes13(0x6b_6365727469666963617465_59),
        ...
        for (uint256 i; i < cabundleLen; i++) {
            if (STRICT) {
                // Bytes(long) - 59
                require(
                    bytes1(input[cursor:cursor + 1]) == 0x59,
                    "invalid cabundle"
                );
            }
            cursor += 1;
            ...
            (serial, issuerHash, pk) = X509.verify(
                icert,
                issuerHash,
                i == 1 ? ROOT_CA_KEY : pk
            );
    ...
    function _verifyPCRs(
        bytes calldata input,
        PcrSelector memory sel
    ) internal pure returns (uint256 adv) {
        if (STRICT) {
            ...
            require(
                bytes9(input[0:9]) == bytes9(0x64_70637273_b0_00_58_30),
                "expected pcrs"
            );
        }
        input = input[9:];
    ...
    function _getUserData(
        bytes calldata input
    ) internal pure returns (UserData memory userdata, uint256 adv) {
        uint256 cursor;

        if (STRICT) {
            // Key: Text(10) "public_key" - 6a_7075626c69635f6b6579
            require(
                bytes11(input[cursor:cursor + 11]) ==
                    bytes11(0x6a_7075626c69635f6b6579),
                "expected public_key"
            );
        }
        cursor += 11;
        ...
        if (STRICT) {
            ...
            require(
                bytes10(input[cursor:cursor + 10]) ==
                    bytes10(0x69_757365725f64617461),
                "expected user_data"
            );
        }
        cursor += 10;
        (
            bytes calldata binding,
            uint256 userdataConsumed
        ...
        if (STRICT) {
            ...
            require(
                bytes6(input[cursor:cursor + 6]) == bytes6(0x65_6e6f6e6365),
                "expected nonce"
            );
        }
        cursor += 6;
        (bytes calldata nonce, uint256 nonceConsumed) = _consumeOptionalBytes(
            input[cursor:]
        );
    ...
    function _consumeOptionalBytes(
        bytes calldata input
    ) internal pure returns (bytes calldata data, uint256 adv) {
        if (input[0] == 0xf6 /* null */ || input[0] == 0x40 /* empty */)
            return (input[0:0], 1);
        if (input[0] == 0x58) {
    ...
    function _getDerUintHdr(
        uint8 leadingByte,
        uint8 len
    ) internal pure returns (bytes memory tag) {
...
library X509 {
    ...
    function parseForIss(
        bytes calldata cert,
        bytes32 expectedIssuerHash
    )
        internal
        view
        returns (
        ...
        )
    {
        ...
        if (STRICT) {
            ...
            require(
                bytes12(cert[cursor:cursor + 12]) ==
                    bytes12(0x30_0a_06_08_2A8648CE3D040303),
                "wrong sig"
            );
        }
        cursor += 12;
        ...
        returns (bytes32 serial, bytes32 iss, bytes32 sub, bytes calldata pk)
    {
        ...
        if (STRICT) {
            ...
            // version 2 - 02
            require(
                bytes5(tbs[cursor:cursor + 5]) == bytes5(0xa0_03_02_01_02),
                "not v2 tbs"
            );
        }
        cursor += 5;
        ...
        if (STRICT) {
            // INTEGER - 02
            require(
                bytes1(tbs[cursor:cursor + 1]) == bytes1(0x02),
                "not serial"
            );
        }
        cursor += 1;
        ...
        (bytes calldata issuer, uint256 issuerSeqLen) = _getVarLenSeq(
            tbs[cursor:]
        );
        cursor += issuerSeqLen;
        ...
        (bytes calldata subject, uint256 subjectSeqLen) = _getVarLenSeq(
            tbs[cursor:]
        );
    ...
    function _extractCN(
        ...
        while (cursor != input.length) {
            if (STRICT) {
                require(
                    bytes1(input[cursor:cursor + 1]) == bytes1(0x31),
                    "not rdn"
                );
            }
            cursor += 1;
            ...
            if (
                bytes7(input[cursor:cursor + 7]) & 0xff00ffffffffff !=
                bytes7(0x30_00_0603550403)
            ) {
                cursor += rdnLen;
                continue;
            }
            ...
            if (STRICT) {
                require(
                    bytes1(input[cursor:cursor + 1]) == bytes1(0x0c),
                    "not cn"
                );
            }
            cursor += 1;
    ...
    function _parseISO8601(bytes13 isodate) internal pure returns (uint256) {
        ...
        uint256 isonums = (uint256(bytes32(isodate)) >> 160) -
            0x303030303030303030303030;
        uint256 ss = ((isonums >> 0x08) & 0xff) *
            10 +
            ((isonums >> 0x00) & 0xff);
        uint256 mm = ((isonums >> 0x18) & 0xff) *
            10 +
            ((isonums >> 0x10) & 0xff);
        uint256 hh = ((isonums >> 0x28) & 0xff) *
            10 +
            ((isonums >> 0x20) & 0xff);
        uint256 dd = ((isonums >> 0x38) & 0xff) *
            10 +
            ((isonums >> 0x30) & 0xff);
        uint256 mM = ((isonums >> 0x48) & 0xff) *
            10 +
            ((isonums >> 0x40) & 0xff);
        uint256 yy = ((isonums >> 0x58) & 0xff) *
            10 +
            ((isonums >> 0x50) & 0xff);
    ...
    function _getVarLenSeq(
        bytes calldata input
    ) internal pure returns (bytes calldata, uint256 adv) {
        uint256 cursor;
        if (STRICT) {
            require(
                bytes1(input[cursor:cursor + 1]) == bytes1(0x30),
                "not seq"
            );
        }
        cursor += 1;
...
library Sig {
    ...
    function verifyP384(
        ...
        if (
            !Sapphire.verify(
                Sapphire.SigningAlg.Secp384r1PrehashedSha384,
                cpk,
                hash,
                "",
                sig
